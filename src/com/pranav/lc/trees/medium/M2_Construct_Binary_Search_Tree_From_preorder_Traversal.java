...
/**
 * @author pranavpatel solution -1 O(N^2) as every time starting from root node
...
public class M2_Construct_Binary_Search_Tree_From_preorder_Traversal {
  ...
  /*
   * public static TreeNode bstFromPreorder(int[] preorder) { TreeNode root = new
   * TreeNode(preorder[0]); for(int i =1;i<preorder.length;i++) {
   * FindTreeNode(root, preorder[i]); } return root; } public static TreeNode
   * FindTreeNode(TreeNode root, int preVal) { if(root == null) { return root =
   * new TreeNode(preVal); } if(root.val> preVal) root.left =
   * FindTreeNode(root.left,preVal); else root.right = FindTreeNode(root.right,
   * preVal); return root; }
   */
  ...
  public static void main(String[] args) {
    ...
    int[] A = { 8, 5, 1, 7, 10, 12 };
    TreePrinter.printNode(bstFromPreorder(A));
  }
